...
export async function POST(req:Request) {
  ...
  await WorkspaceRepository.create(user.id, body.businessName, body.businessType ?? "business");
...
Meta principal: ${body.goal ?? "No especificada"}`
  ...
  await TaskRepository.create(user.id, `Configurar ALMA para ${body.businessName}`);
