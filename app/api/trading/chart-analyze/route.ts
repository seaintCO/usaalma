...
async function post(req: Request) {
  ...
  const question = String(
    form.get("question") ||
      "Analyze this chart. Where should I look for calls or puts?",
  );

  if (!file) {
    return NextResponse.json(
      { error: "Missing chart screenshot." },
      { status: 400 },
    );
  }
  ...
  if (!file.type.startsWith("image/")) {
    return NextResponse.json(
      { error: "Please upload a chart screenshot image." },
      { status: 400 },
    );
  }
  ...
  const sourceImage = await ImageRepository.create(user.id, {
    prompt: "Trader chart screenshot",
  ...
  const result: any = await withUsageReservation(
    {
      ...
      idempotencyKey: `chart:${req.headers.get("x-idempotency-key") ?? crypto.randomUUID()}`,
    },
    () =>
