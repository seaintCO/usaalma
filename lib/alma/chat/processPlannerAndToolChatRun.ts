...
export type PlannerToolChatRunResult =
  ...
  | { handled: false; systemPrompt: string; tracking: ChatRunTrackingContext | null };

function localized(input: PlannerToolChatRunInput, english: string, spanish: string) {
  return input.language === "en" ? english : spanish;
}

async function emit(callback: ChatRunProgressCallback | undefined, event: Parameters<ChatRunProgressCallback>[0]) {
  if (!callback) return;
  try {
...
async function persistAssistant(input: PlannerToolChatRunInput, content: string) {
...
async function completeHandled(
  ...
  await emit(input.onProgress, result.ok
    ? { type: "completed", result }
    : { type: "failed", error: result });
  return { handled: true as const, result };
}

function plannedReply(input: PlannerToolChatRunInput, planned: { goal: string; steps: Array<{ label: string; result?: { message?: string } }> }) {
  const steps = planned.steps.map((step, index) => `${index + 1}. ${step.label} — ${input.language === "en" ? "Completed" : step.result?.message || "Completado"}`).join("\n");
...
export async function processPlannerAndToolChatRun(input: PlannerToolChatRunInput): Promise<PlannerToolChatRunResult> {
    ...
    if (planned) {
      ...
      if (input.onProgress) {
        for (const word of reply.split(" ")) {
          await emit(input.onProgress, { type: "text_delta", delta: `${word} ` });
          await new Promise((resolve) => setTimeout(resolve, 15));
        }
      }
      await persistAssistant(input, reply);
      const result: ChatRunResult = { ok: true, responseType: "planned_execution", route: "planned_execution", finalContent: reply, tracking };
      return completeHandled(input, tracking, result, true, "ALMA completed a planned execution.");
    }
  } catch (error) {
  ...
  if (detectedIntent === "finance_analysis") {
    await emit(input.onProgress, { type: "status", message: localized(input, "Preparing market analysis...\n\n", "Preparando análisis de mercado...\n\n") });
    try {
      ...
      const reply = result.output_text || localized(input, "No market analysis available.", "No hay análisis de mercado disponible.");
      await persistAssistant(input, reply);
      await emit(input.onProgress, { type: "text_delta", delta: reply });
      const terminal: ChatRunResult = { ok: true, responseType: "finance_analysis", route: "finance_analysis", finalContent: reply, tracking };
      return completeHandled(input, tracking, terminal, true, "ALMA completed market analysis.");
    } catch {
      const reply = localized(input, "ALMA could not generate market analysis right now.", "ALMA no pudo generar el análisis de mercado ahora.");
      await persistAssistant(input, reply);
      await emit(input.onProgress, { type: "text_delta", delta: reply });
      const terminal: ChatRunResult = { ok: false, responseType: "finance_analysis", route: "finance_analysis", finalContent: reply, tracking, code: "model_request_failed", message: reply };
      return completeHandled(input, tracking, terminal, false, "ALMA market analysis failed.", reply);
    }
  }
  ...
  const [integrationContext, documentContext, workspaceContext] = await Promise.all([
...
`;
  ...
  try {
    firstResponse = await client.responses.create({
      model: chooseAlmaModel(input.userMessage, "auto"),
      input: [{ role: "system", content: systemPrompt }, { role: "user", content: input.userMessage }],
  ...
  try {
    const finalStream = await client.responses.create({
      model: chooseAlmaModel(input.userMessage, "auto"),
      stream: true,
      input: [{ role: "system", content: systemPrompt }, { role: "user", content: input.userMessage }, ...toolCalls, ...toolResults],
    });
    for await (const event of finalStream as any) {
      if (event.type === "response.output_text.delta") {
        fullReply += event.delta;
        await emit(input.onProgress, { type: "text_delta", delta: event.delta });
      }
    }
    await persistAssistant(input, fullReply);
    const terminal: ChatRunResult = { ok: true, responseType: "tool_assisted", route: "tool_assisted", finalContent: fullReply, tracking, tools: { calls: toolResults.length, names: toolCalls.map((call: any) => call.name) } };
    return completeHandled(input, tracking, terminal, true, "ALMA completed a tool-assisted chat execution.");
  } catch {
    const reply = localized(input, "\n\nALMA had an error executing the tool.", "\n\nALMA tuvo un error ejecutando la herramienta.");
    await persistAssistant(input, reply);
    await emit(input.onProgress, { type: "text_delta", delta: reply });
    const terminal: ChatRunResult = { ok: false, responseType: "tool_assisted", route: "tool_assisted", finalContent: reply, tracking, code: "tool_execution_failed", message: reply };
    return completeHandled(input, tracking, terminal, false, "ALMA tool-assisted chat execution failed.", "Streaming tool response failed");
  }
}
