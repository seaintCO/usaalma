...
const PRICE_MAP:Record<string,string | undefined> = {
  ...
  personal: process.env.STRIPE_PRICE_STARTER || process.env.STRIPE_PRICE_PERSONAL,
};
...
export async function POST(req:Request) {
  try {
    ...
    if (!priceId) {
      return NextResponse.json({ error:`Stripe price for ${plan} is not configured.` }, { status:500 });
    }
