    ...
    <main className="flex h-screen w-full overflow-hidden bg-white text-[#111111]">
      <aside className="hidden h-full w-64 flex-col border-r border-[#E5E7EB] bg-[#F7F7F8] md:flex">
        ...
        <div className="flex-1 overflow-y-auto px-3 pb-4 text-sm">
          ...
          {["Resumen reunión Q3", "Ideas campaña marketing", "Factura cliente Acme"].map((item) => (
            <a key={item} className="block truncate rounded-lg px-2 py-1.5 text-[#6B7280] hover:bg-gray-200 hover:text-black">{item}</a>
          ))}
        ...
        <div className="absolute bottom-0 w-full bg-gradient-to-t from-white via-white to-transparent px-4 pb-6 pt-10 md:px-8">
          <div className="mx-auto flex w-full max-w-3xl flex-col gap-4">
            ...
            <div className="relative flex flex-col overflow-hidden rounded-2xl border border-[#E5E7EB] bg-[#F7F7F8] shadow-sm">
              <textarea rows={1} placeholder="Pregúntale cualquier cosa..." className="max-h-32 w-full resize-none bg-transparent p-4 pb-12 text-base outline-none placeholder:text-gray-400" />
