...
async function post(request: Request) {
  ...
  if (!body.sourceImageId || !String(body.prompt ?? "").trim())
    return NextResponse.json(
      { error: "Source image and edit prompt are required." },
      { status: 400 },
    );
  const source = await ImageRepository.get(user.id, String(body.sourceImageId));
  ...
  const result = await generateImageTool(
    ...
    undefined,
    {
      ...
      sourceImageBase64: source.image_base64,
