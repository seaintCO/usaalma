...
export class AgentService {
  ...
  static async startExecution(input: Omit<AgentExecutionInput, "agentId">) {
    ...
    if (input.idempotencyKey) {
      const existing = await AgentExecutionRepository.findByIdempotency({
        agentId: agent.id,
        userId: input.userId,
        idempotencyKey: input.idempotencyKey,
      });
      if (existing) return { agent, execution: existing };
    }
    const execution = await AgentExecutionRepository.create({
      ...input,
      agentId: agent.id,
    });
    await AgentActivityRepository.create({
      agentId: agent.id,
      userId: input.userId,
      executionId: execution.id,
      eventType: "execution_started",
      summary: input.goal || "ALMA started an execution.",
      metadata: {
  ...
  static async completeExecution(input: {
    agentId: string;
    executionId: string;
    userId: string;
    success: boolean;
    ...
    await AgentExecutionRepository.complete(
      input.executionId,
    ...
    await AgentActivityRepository.create({
      ...
      eventType: input.success ? "execution_completed" : "execution_failed",
      summary: input.summary,
      metadata: input.result ?? {},
    });
  }

  static async recordStep(input: {
    executionId: string;
    sequence: number;
    kind: "plan" | "tool" | "approval" | "verification" | "reflection";
    toolName?: string;
    success?: boolean;
  ...
  static async finishStep(input: {
    stepId: string;
    success: boolean;
    output?: Record<string, unknown>;
    error?: string | null;
  }) {
    return AgentExecutionRepository.finishStep(
      input.stepId,
      input.success ? "completed" : "failed",
      input.output,
      input.error,
