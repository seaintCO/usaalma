...
export default function FitnessPage() {
  ...
  async function searchFood() {
    ...
    const data = await fetch(
      `/api/fitness/v2/food-search?q=${encodeURIComponent(query)}`,
    ).then((r) => r.json());
  ...
  return (
    ...
    >
      <div className="px-4 py-8 text-[#111111] md:px-6 md:py-10">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-8">
            ...
            <p className="mt-3 text-[#6B7280]">
              Food log, goals, meal planning, progress, and AI coaching.
            </p>
          </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[420px_1fr]">
            <section className="space-y-6">
              <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-5">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <input
                    ...
                    className="rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                  />
                  <input
                    value={goals.protein}
                    onChange={(e) =>
                  ...
                  <input
                    value={goals.fat}
                    onChange={(e) =>
                      setGoals({ ...goals, fat: e.target.value })
                    }
                    placeholder="Fat"
                    className="rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                  />
                ...
                <input
                  ...
                  onChange={(e) =>
                    setGoals({ ...goals, weight_goal: e.target.value })
                  }
                  placeholder="Weight goal"
                  className="mt-3 w-full rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                />
                <button
                  onClick={saveGoals}
                  className="mt-3 flex w-full items-center justify-center gap-2 rounded-2xl bg-black py-3 text-white"
                >
              ...
              <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-5">
                <div className="mb-4 flex items-center gap-2 font-medium">
                  <Apple className="h-4 w-4" /> Food Log
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                  />
                  <button
                    onClick={searchFood}
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <input
                    ...
                    onChange={(e) =>
                      setManual({ ...manual, food_name: e.target.value })
                    }
                    placeholder="Food"
                    className="rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                  />
                  ...
                  <input
                    value={manual.fat}
                    onChange={(e) =>
                      setManual({ ...manual, fat: e.target.value })
                    }
                    placeholder="Fat"
                    className="rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                  />
                  <input
                    value={manual.serving_qty}
                    onChange={(e) =>
                      setManual({ ...manual, serving_qty: e.target.value })
                    }
                    placeholder="Serving"
                    className="rounded-2xl bg-[#F7F7F8] px-4 py-3 outline-none"
                  />
            ...
            <section className="space-y-6">
                    ...
                    <div key={name} className="rounded-2xl bg-[#F7F7F8] p-4">
                      ...
                      <div className="mt-3 h-2 rounded-full bg-white">
                        <div
                          ...
                          style={{
                            width: `${pct(Number(value), Number(goal))}%`,
                          }}
                        />
                      </div>
                    </div>
              ...
              <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-6">
                ...
                <div className="mt-4 space-y-3">
                  {foods.length === 0 && (
                    <div className="rounded-2xl bg-[#F7F7F8] p-5 text-[#6B7280]">
                  ...
                  {foods.map((food: any) => (
                    <div
                      key={food.id}
                      className="flex items-center justify-between gap-3 rounded-2xl bg-[#F7F7F8] p-4"
                    >
                      <div>
                        <div className="font-medium">{food.food_name}</div>
              ...
              {editing && (
                <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-6">
                  ...
                  <button
                    onClick={updateFood}
                    className="mt-3 rounded-2xl bg-black px-5 py-3 text-white"
                  >
                    Save edit
                  </button>
                </div>
              ...
              <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-6">
                <h2 className="flex items-center gap-2 text-2xl font-medium">
                  <Calendar className="h-5 w-5" /> Meal Planner + AI Coach
                </h2>
                <button
                  onClick={generatePlan}
                  className="mt-4 flex items-center gap-2 rounded-2xl bg-black px-5 py-3 text-white"
                >
                  <Sparkles className="h-4 w-4" />{" "}
                  {loading ? "Generating..." : "Generate plan"}
                </button>
                <pre className="mt-5 whitespace-pre-wrap rounded-2xl bg-[#F7F7F8] p-5 text-sm leading-7">
                  {plan || "Generate a plan based on your goals."}
                </pre>
              </div>
              ...
              <div className="rounded-[2rem] border border-[#E5E7EB] bg-white p-6">
                ...
                <div className="mt-4 rounded-2xl bg-[#F7F7F8] p-5 text-sm leading-7">
                  Eggs, Greek yogurt, chicken breast, lean beef, rice, potatoes,
                  vegetables, fruit, protein powder.
                </div>
              </div>
            </section>
          </div>
