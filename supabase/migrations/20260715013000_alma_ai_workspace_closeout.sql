...
alter table public.creative_campaigns add column if not exists archived_at timestamptz;
create index if not exists creative_campaigns_user_active_idx on public.creative_campaigns(user_id,updated_at desc) where archived_at is null;
