...
type Project = {
  id: string;
  title: string;
  launch_data: Record<string, string>;
};
const fields = [
  "product",
  "description",
...
const labels: Record<string, [string, string]> = {
  ...
  offer: ["Offer", "Oferta"],
  competitors: ["Competitors", "Competidores"],
  brandMessaging: ["Brand messaging", "Mensaje de marca"],
  salesAngle: ["Sales angle", "Ángulo de ventas"],
  roadmap: ["90-day roadmap", "Ruta de 90 días"],
  checklist: ["Launch checklist", "Lista de lanzamiento"],
  calendar: ["Campaign calendar", "Calendario de campaña"],
  socialStrategy: ["Social strategy", "Estrategia social"],
  emailStrategy: ["Email strategy", "Estrategia de email"],
  partnershipStrategy: ["Partnership strategy", "Estrategia de alianzas"],
  landingPage: ["Landing page", "Landing page"],
  salesPage: ["Sales page", "Página de ventas"],
  headline: ["Headline", "Titular"],
  cta: ["CTA", "CTA"],
  emailSequence: ["Email sequence", "Secuencia de email"],
  ads: ["Ads", "Anuncios"],
  tiktokIdeas: ["TikTok ideas", "Ideas TikTok"],
  instagramIdeas: ["Instagram ideas", "Ideas Instagram"],
  linkedinIdeas: ["LinkedIn ideas", "Ideas LinkedIn"],
};
const text = {
  en: {
    title: "Launch Studio",
    subtitle: "Your company launch operating system.",
    new: "New project",
    save: "Save",
    duplicate: "Duplicate",
    archive: "Archive",
    delete: "Delete",
    ask: "Build this launch plan with ALMA",
    visuals: "Create visuals in Creative Studio",
    empty: "No launch projects yet.",
    copy: "Copy",
    markdown: "Markdown",
    pdf: "PDF",
  ...
  es: {
    title: "Launch Studio",
    subtitle: "Tu sistema operativo para lanzar una empresa.",
    new: "Nuevo proyecto",
    save: "Guardar",
    duplicate: "Duplicar",
    archive: "Archivar",
    delete: "Eliminar",
    ask: "Crear este plan con ALMA",
    visuals: "Crear visuales en Creative Studio",
    empty: "Aún no hay proyectos.",
    copy: "Copiar",
    markdown: "Markdown",
    pdf: "PDF",
    error: "La solicitud de proyecto falló.",
  },
};
const blank = () =>
...
export default function LaunchStudioPage() {
  ...
  const select = async (id: string) => {
    ...
    setProject(next);
    setTitle(next.title);
    setData({ ...blank(), ...(next.launch_data ?? {}) });
  };
  const reset = () => {
    setProject(null);
    setTitle("");
  ...
  const downloadMarkdown = () => {
    ...
    link.href = url;
    link.download = `${title || "launch-plan"}.md`;
    link.click();
  ...
  const downloadPdf = async () => {
    ...
    const doc = new jsPDF();
    doc.text(doc.splitTextToSize(content(), 180), 15, 15);
    doc.save(`${title || "launch-plan"}.pdf`);
  };
  const askAlma = () => {
    const prompt =
      language === "es"
        ? `Crea un plan de lanzamiento para ${title || "mi empresa"}. ${data.description || ""}`
          ...
          <div className="mt-7 grid gap-5 lg:grid-cols-[260px_1fr]">
              ...
              {projects.length === 0 ? (
                ...
                projects.map((item) => (
                  <button
                    ...
                    className={`mt-2 block w-full truncate rounded-xl p-3 text-left text-sm ${project?.id === item.id ? "bg-black text-white" : "bg-[#F7F7F8]"}`}
                  >
                    {item.title}
                  </button>
                ))
            ...
            <section className="rounded-2xl border bg-white p-5">
              <input
                ...
                placeholder={
                  language === "es" ? "Nombre de empresa" : "Company name"
                }
                className="w-full rounded-xl bg-[#F7F7F8] p-3 text-xl font-medium"
              />
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {fields.map((field) => (
                  <label key={field} className="text-sm font-medium">
                    {labels[field][language === "es" ? 1 : 0]}
                    <textarea
                      value={data[field] || ""}
                      onChange={(event) =>
                        setData({ ...data, [field]: event.target.value })
                      }
                      className="mt-1 min-h-24 w-full rounded-xl bg-[#F7F7F8] p-3 text-sm font-normal"
                    />
                  </label>
                ))}
              </div>
              <div className="mt-6 flex flex-wrap gap-2">
                ...
                <a
                  href={WORKSPACE_ROUTES.creative}
                  className="rounded-full border px-4 py-2 text-sm"
                >
                  {t.visuals}
                </a>
                {project && (
                  <>
                    <button
                      onClick={async () => {
                        const response = await fetch(
                          `/api/launch-studio/workspace/${project.id}/duplicate`,
                          { method: "POST" },
                        );
                        if (response.ok)
                          await select((await response.json()).project.id);
                      }}
                      className="rounded-full border px-4 py-2 text-sm"
                    >
                      {t.duplicate}
                    ...
                    <button
                      onClick={async () => {
                        await fetch(
                          `/api/launch-studio/workspace/${project.id}`,
                          {
                            method: "PATCH",
                            headers: { "Content-Type": "application/json" },
